...
namespace DropParty
{
    public class Player : IPlayer, ICollidable, IDummyPlayer
    {
        ...
        public UInt16 playerKills { get; set; }

        public Player(IMap mapBullets, IBase Base, IControllable controllable, Color baseColor, PlayerHandler playerHandler)
        {
        ...
        public void OnCollide(List<ICollidable> collidedType)
        {
            ...
            foreach (var collision in collidedType)
            {
                if (collision is Bullet || collision is Player || collision is Map)
                {
                    ...
                    {
                        Bullet bullet = (Bullet) collision;
                        playerHandler.UpdatePlayerKills(bullet.ShootingPlayerName);
                    }
                    if (!(collision is Bullet))
                    {
                        Spawn();
        ...
        public void Shoot(float degrees)
        {
            ...
            Bullet myBullet = new Bullet(myVelocity, myMapRef, (int)BulletSpeed, PlayerName);
